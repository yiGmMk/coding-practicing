...
// @lc code=start
// 超内存了
func getSkylineOutOfMemory(buildings [][]int) (res [][]int) {
	sort.Slice(buildings, func(i, j int) bool { return buildings[i][0] < buildings[j][0] })
	l, r := buildings[0][0], buildings[len(buildings)-1][1]
	high := make([]int, r+1)
	for _, v := range buildings {
		for st, ed, h := v[0], v[1], v[2]; st < ed; st++ {
			if h > high[st] {
				high[st] = h
			}
		}
	...
	pre := 0
	for st, ed := l, r; st < ed; st++ {
		if high[st] != pre {
			res = append(res, []int{st, high[st]})
			pre = high[st]
		}
	}

 ...
 */
// ------------------可优化,实际只需要存最后一个节点就行------------------------
func handler(node, child, next *Node) {
	ns := []*Node{}
	for pc := child; pc != nil; pc = pc.Next {
		if pc.Child == nil {
			ns = append(ns, pc)
		} else {
			handler(pc, pc.Child, pc.Next)
			pc.Child = nil // 删除上下层关系
		}
	}
	node.Next = child
	child.Prev = node

	ns[len(ns)-1].Next = next
	if next != nil { // next 可能为空
		next.Prev = ns[len(ns)-1]
	}

}

// 每个child子串只存最后一个节点
func handlerOptimize(node, child, next *Node) {
	last := node
	for pc := child; pc != nil; pc = pc.Next {
		if pc.Child == nil {
			if pc != nil {
			...
			handlerOptimize(pc, pc.Child, pc.Next)
			pc.Child = nil // 删除上下层关系
		}
	}
	node.Next = child
...
func flatten(root *Node) *Node {
	...
	for p != nil {
		if p.Child != nil {
			handlerOptimize(p, p.Child, p.Next)
			p.Child = nil // 删除上下层关系
		}
		p = p.Next
	}
